...
//go:embed static_binary/linux/ctags
//go:embed static_binary/linux/readtags
//go:embed static_binary/linux/global
//go:embed static_binary/linux/gtags
...
func extractBinary(name, destDir string) error {
	// 从embed FS中读取二进制文件
	data, err := staticBinaries.ReadFile("static_binary/linux/" + name)
	if err != nil {
